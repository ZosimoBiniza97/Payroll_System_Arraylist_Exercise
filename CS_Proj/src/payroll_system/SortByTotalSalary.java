...
public class SortByTotalSalary {
	...
	public void quickSort(LinkedList<list_model> list,int swtch) {
	  ...
	  public float findKthLargest(LinkedList<list_model> list, int k,int swtch) {
	    // f([11,12,13], 1) => 13
	    // f([11,12,13], 2) => 12
	    ...
	    return list.get(start).getTotalpay();
	  }
	  ...
	  private int partition(LinkedList<list_model> list, int start, int end,int swtch){
	    ...
	    float pivot = list.get(start).getTotalpay();
	    int i = start;
	    for (int j = start + 1; j < end; ++j) {
	    	if(swtch == 1){
	    		if (list.get(j).getTotalpay() <= pivot) {
	    			i += 1;
	    			swap(list, i, j);
	    		}
	      }
