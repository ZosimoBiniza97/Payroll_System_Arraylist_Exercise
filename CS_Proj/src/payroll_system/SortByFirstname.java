	...
	public void quickSort(LinkedList<list_model> list,int swtch) {
	  ...
	  public String findKthLargest(LinkedList<list_model> list, int k,int swtch) {
	    ...
	    while (start + 1 < end) {
	      ...
	      if (i == targetIdx) {
	        return list.get(i).getFirst_name();
	      } else if (i < targetIdx) {
	        start = i + 1;
	      } else {
	        end = i;
	    ...
	    return list.get(start).getFirst_name();
	  }
	  ...
	  private int partition(LinkedList<list_model> list, int start, int end,int swtch){
	    ...
	    String pivot = list.get(start).getFirst_name();
	    int i = start;
	    int compare;
	    for (int j = start + 1; j < end; ++j)
	    { if(swtch == 1)
    	{
    	int length = list.get(j).getFirst_name().length();
    	...
    	for(int z=0; z<min;z++){
		if (list.get(j).getFirst_name().toUpperCase().charAt(z) < pivot.toUpperCase().charAt(z)) {
			i += 1;
			swap(list, i, j);
			break;
		}
		else if(list.get(j).getFirst_name().toUpperCase().charAt(z) > pivot.toUpperCase().charAt(z))
		{
			break;
		}
    	}
