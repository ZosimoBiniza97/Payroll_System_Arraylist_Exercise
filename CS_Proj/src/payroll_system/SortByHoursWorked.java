...
public class SortByHoursWorked {
	...
	public void quickSort(LinkedList<list_model> list,int swtch) {
	  ...
	  public int findKthLargest(LinkedList<list_model> list, int k,int swtch) {
	    ...
	    while (start + 1 < end) {
	      ...
	      if (i == targetIdx) {
	        return list.get(i).getHoursworked();
	      } else if (i < targetIdx) {
	        start = i + 1;
	      } else {
	        end = i;
	    ...
	    return list.get(start).getHoursworked();
	  }
	  ...
	  private int partition(LinkedList<list_model> list, int start, int end,int swtch){
	    ...
	    int pivot = list.get(start).getHoursworked();
	    int i = start;
	    for (int j = start + 1; j < end; ++j) {
	    	if(swtch == 1){
	    		if (list.get(j).getHoursworked() <= pivot) {
	    			i += 1;
	    			swap(list, i, j);
	    		}
	      }
